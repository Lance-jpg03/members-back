...
// Only enable Gmail transporter in development
if (process.env.NODE_ENV !== "production") {
  ...
  console.warn("⚠️ Email transporter is disabled in production (Render)");
}
...
async function sendTicketNotification({ ticketId, name, email, department, urgency, description, assignee }) {
  ...
  if (!transporter) {
    console.warn("⚠️ Skipping sendTicketNotification — no transporter configured.");
    return;
  }
...
async function sendRequesterConfirmation({ ticketId, name, email, department, urgency, description, assignee }) {
  ...
  if (!transporter) {
    console.warn("⚠️ Skipping sendRequesterConfirmation — no transporter configured.");
    return;
  }
